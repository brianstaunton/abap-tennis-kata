...
CLASS lcl_tennis_game2 IMPLEMENTATION.
  ...
  METHOD lif_tennis_game~score.
    "   Handle Win
    IF mv_p1_point > 3 AND ( mv_p1_point - mv_p2_point ) > 1.
      rv_score = |Win for { mv_player1_name }|.
    ELSEIF mv_p2_point > 3 AND ( mv_p2_point - mv_p1_point ) >  1.
      rv_score = |Win for { mv_player2_name }|.
    ELSE.
      " Handle evens
      IF mv_p1_point = mv_p2_point.
        IF mv_p1_point < 3.
          rv_score = score_as_string( mv_p1_point ) && |-All|.
        ELSE.
          rv_score = |Deuce|.
        ENDIF.
      ELSE.
        " Handle Player up
        IF mv_p1_point > mv_p2_point AND mv_p2_point >  2.
          rv_score = |Advantage { mv_player1_name }|.
        ELSEIF mv_p2_point > mv_p1_point AND mv_p1_point > 2.
          rv_score = |Advantage { mv_player2_name }|.
        ELSE.
          rv_score = score_as_string( mv_p1_point ) && |-| && score_as_string( mv_p2_point ).
        ENDIF.
  ...
  METHOD set_p1_score.
    mv_p1_point = mv_p1_point + iv_number .
  ENDMETHOD.

  METHOD set_p2_score.
    mv_p2_point = mv_p2_point + iv_number .
  ENDMETHOD.
